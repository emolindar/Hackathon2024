...
import hashing
import user_authentication.password_db as password_db
...
def register_user(username, password):
    ...
    '''
    if password_db.get_user(username) is not None:
    ...
    success = password_db.create_user(username, hashed_password)
    if success:
...
def login_user(username, password):
    ...
    - tuple: (bool, str) indicating success/failure and a message explaining the outcome.
    '''
    user_record = password_db.get_user(username)
