...
# Model 1
formula = 'NumberOfCustomers ~ ' + ' + '.join([col for col in trainData.columns if col != 'NumberOfCustomers' and col != 'Date' and col != 'TimeOfDay'])
...
# Plot for Model 1
sns.scatterplot(x='Profits', y='NumberOfCustomers', data=testData)
...
mae1 = mean_absolute_error(testData['NumberOfCustomers'], predictions1)
with open('model1_summary.txt', 'w') as file:
    file.write(f'Coefficients:\n{lr1.params}\n\n')
    file.write(f'Mean Absolute Error (MAE): {mae1}\n')

# Model 2
columns = ["NumberOfEmployees", "Satisfaction", "Profits", "NumberOfCustomers"]
trainData2 = trainData[columns]
testData2 = testData[columns]
formula2 = 'Profits ~ NumberOfEmployees + Satisfaction + NumberOfCustomers'
...
fig, ax = plt.subplots()
sns.scatterplot(x='NumberOfEmployees', y='Profits', data=testData2, ax=ax)
sns.lineplot(x='NumberOfEmployees', y=predictions2, data=testData2, color='red', ax=ax)
sns.regplot(x='NumberOfEmployees', y='Profits', data=testData2, scatter=False, color='blue', ax=ax)
